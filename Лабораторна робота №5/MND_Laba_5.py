...
def check(X, Y, B, n, m):
    ...
    print('Fp =', F_p)
    print('F_t =', f_t)
    if F_p < f_t or len(final_k)!=1:# Додали додаткову умову перевірки на адекватність системи: якщо кількість значимих кооефіцєнтів дорівнює один - то модель неадекватна
