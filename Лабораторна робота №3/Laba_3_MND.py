...
from scipy.stats import f, t """ Этот модуль содержит большое количество вероятностных распределений, а также растущую библиотеку статистических функций.
scipy.stats.f - Непрерывная случайная величина F. В качестве экземпляра rv_continuousкласса fобъект наследует от него набор универсальных методови дополняет их деталями, специфичными для этого конкретного дистрибутива.
В данному коді було застосованно у 151 рядку.
scipy.stats.t - Непрерывная случайная величина Стьюдента. В качестве экземпляра rv_continuousкласса tобъект наследует от него набор универсальных методов  и дополняет их деталями, специфичными для этого конкретного дистрибутива
В данному коді було застосованно у 116 рядку.."""
